...
//                }
    ...
    public function getDevolucionesX() {
        try {
            extract(filter_input_array(INPUT_POST));
            $data = $this->almacen_model->getDevolucionesX($INICIO, $FIN, $PROYECTO, $ALMACEN);
            print json_encode($data);
        } catch (Exception $exc) {
            echo $exc->getTraceAsString();
        }
...
//            $pdf->Line($x1, $y1, $x2, $y2);            
    ...
    public function getReporteDevolucionesX() {
        try {
            extract($this->input->post());
            date_default_timezone_set('America/Mexico_City');
            $pdf = new FPDF('L');
            ...
            $pdf->SetX(110);
            $pdf->SetFont('Arial', '', 25);
            $pdf->Cell(100, 15, 'DEVOLUCIONES', 0, 0, 'L');
            $pdf->SetX(238);
            $pdf->SetFont('Arial', '', 8);
            $pdf->Cell(100, 15, date("d/m/Y ") . ' ' . date("h:i:s a"), 0, 1, 'L');
            $pdf->SetY(25);
            ...
            $pdf->SetY($Y);
            $pdf->SetX($space);
            $pdf->Multicell(40, 5, 'ALMACEN', 1, 'C');
            $pdf->SetY($Y);
            $pdf->SetX($space += 40);
            $pdf->Multicell(35, 5, 'ORDEN DE COMPRA', 1, 'C');
            $YZ = $pdf->GetY();
            $pdf->SetY($Y);
            $pdf->SetX($space += 35);
            $pdf->Multicell(35, 5, 'FACTURA', 1, 'C');
            $YZ = ($pdf->GetY() > $YZ) ? $pdf->GetY() : $YZ;
            /*             * *************************FIN ENCABEZADO************************ */

            /*             * ****************************DETALLE***************************** */

//          $rows = $this->almacen_model->getEntradasXFecha($INICIO, $FIN, $PROYECTO, $ALMACEN, $PROVEEDOR, $PRODUCTO);
            $rows = $this->almacen_model->getDevolucionesX($INICIO, $FIN, $PROYECTO, $ALMACEN);
            $YY = $YZ;
            $pdf->SetY($YY);
            foreach ($rows as $key => $value) {
                $pdf->SetX(5);
                $pdf->MultiCell(40, 5, $value->ALMACEN, 1, 'C');
                $pdf->SetY($YY);
                $pdf->SetX(45);
                $pdf->MultiCell(35, 5, $value->{"ORDEN DE COMPRA DE REFERENCIA"}, 1, 'C');
                $pdf->SetY($YY);
                $pdf->SetX(80);
                $pdf->MultiCell(50, 5, ($value->{"ORDEN DE FACTURA DE REFERENCIA"} !== NULL && $value->{"ORDEN DE FACTURA DE REFERENCIA"} !== '') ? $value->{"ORDEN DE FACTURA DE REFERENCIA"} : "NO ESPECÍFICA", 1, 'C');
            }

            $pdf->SetFont('Arial', 'B', 6.5);
            $pages = 1;
            /*             * ***************************FIN DETALLE************************** */
            if (!file_exists('uploads/Devoluciones')) {
                mkdir('uploads/Devoluciones', 0777, true);
            }
            $file_name = "DEVOLUCION";
            $url = 'uploads/Devoluciones/' . $file_name . '.pdf';
    ...
    public function onGenerarCodigoDeBarra() {
        try {
            extract($this->input->post());
            $data = $this->productos_model->getProductoByID($ID);
            $producto = $data[0];
            ...
            //A set
            $code = 'CODE 128';
            $pdf->Code128(5, 5, str_pad($producto->ID, 12, '0', STR_PAD_LEFT), 20, 15);
            $pdf->SetFont('Arial', '', 7);
            $pdf->SetY(20);
            $pdf->SetX(5);
            $pdf->MultiCell(19.75, 3, str_pad($producto->ID, 12, '0', STR_PAD_LEFT), 1, 'J'); 

            if (!file_exists('uploads/Codigos')) {
                mkdir('uploads/Codigos', 0777, true);
            }
            $file_name = $producto->ID . "";
            $url = 'uploads/Codigos/ENTRADA_' . $file_name . '.pdf';
            $pdf->Output($url);
            print base_url() . $url;
        } catch (Exception $exc) {
            echo $exc->getTraceAsString();
    ...
    public function onGenerarCodigoDeBarraSalida() {
        try {
            extract($this->input->post());
            $data = $this->productos_model->getProductoByID($ID);
            $producto = $data[0];
            ...
            //A set
            $code = 'CODE 128';
            $pdf->Code128(5, 5, str_pad($producto->ID, 15, '0', STR_PAD_LEFT), 20, 15);
            $pdf->SetFont('Arial', '', 7);
            $pdf->SetY(20);
            $pdf->SetX(5);
            $pdf->MultiCell(19.75, 3, str_pad($producto->ID, 15, '0', STR_PAD_LEFT), 1, 'J'); 

            if (!file_exists('uploads/Codigos')) {
                mkdir('uploads/Codigos', 0777, true);
            }
            $file_name = $producto->ID . "";
            $url = 'uploads/Codigos/SALIDA_' . $file_name . '.pdf';
            $pdf->Output($url);
            print base_url() . $url;
        } catch (Exception $exc) {
            echo $exc->getTraceAsString();
...
//____________________________ Extension du constructeur _______________________
    ...
    function __construct($orientation = 'P', $unit = 'mm', $format = 'A4') {
        ...
        parent::__construct($orientation, $unit, array(50/**/, 50/**/));
